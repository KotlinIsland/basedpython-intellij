...
- [x] Inlay hint toggle settings (param names, types, return types). The return-types toggle is forward-looking: `by ruff/0.0.1` emits no `-> T` hints, only `: T` on bindings, `name=` on arguments and adornments like `override `. Kind maps straight from LSP for the first two; a return hint is recognised by its leading arrow, since LSP has no kind for one (`lsp.inlay.ByInlayHints.kindOf`)
