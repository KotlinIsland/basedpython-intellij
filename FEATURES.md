...
- [x] **`print(…)` → log point** (`inspections.logpoint`) — the counterpart of Kotlin's "'println' call can be replaced with a logpoint": a weak warning on a `print` statement whose quick fix deletes the call and creates a `.by` line breakpoint that logs the argument and does not suspend. Nothing of the IDE's own logpoints feature could be reused. All of it — the inter-line gutter affordance, the inline expression editor, `AddLogpoint` (Ctrl+Alt+F8), and the Java/Kotlin versions of this inspection — lives in `intellij.debugger.logpoints.{shared,frontend,backend,java}`, modules bundled with **IntelliJ IDEA's Java plugin**; PyCharm 2026.2.1 ships none of them (checked in both distributions), and the one per-language hook, `com.intellij.xdebugger.logpoints.editorsProviderFactory`, sits in a module declared `visibility="internal"`. What the platform *does* have, in every IDE, is the mechanism underneath: `XLineBreakpoint.logExpressionObject`, which the DAP client sends on as `logMessage` (wrapped as `{expr}`), and which debugpy turns back into a print inside the debuggee — so the output arrives on the same run console the deleted `print` was writing to. In IDEA the inter-line gutter is offered in `.by` files anyway: `XLogpointInterLineConfigurationProvider` keys only off the `xdebugger.logpoints.enabled` registry and whether a line already has one, with no language gate, so a click there creates a logpoint on `ByLineBreakpointType`
  - The log point goes on the *following* statement, not where the call was: a breakpoint fires before its line runs, and the `print`'s line is being deleted. So the fix is offered only while that follower is in the same block — a `print` at the end of a function is followed by a line that runs at some entirely different time. Leaving a `pass` behind as an anchor was the obvious alternative and does not work: `pass` emits no bytecode, so the line carries no trace event and the breakpoint would never bind
