...
internal object ByTestNodeActions {
     ...
     */
    fun run(project: Project, target: String?, executor: Executor) {
        val paths = target?.let(ByTestNodes::sourceTarget).orEmpty()
    ...
    /**
     * Opens the `.by` declaration [target] was collected from, and reports whether it could.
     *
     ...
     */
    fun navigate(project: Project, target: String?): Boolean {
        val location = target?.let(ByTestLocations::parse) ?: return false
        val file = ByTestSources.findSourceFile(project, location.file) ?: return false
