 ...
 */
class ByPytestTest : TestCase() {

    fun `test no targets runs the whole project`() {
        assertEquals(listOf("pytest", "-v"), ByPytest.arguments(""))
        assertEquals(listOf("pytest", "-v"), ByPytest.arguments("   "))
    }

    fun `test a file target is rewritten onto the transpiled output`() {
        assertEquals(
    ...
    fun `test a node id keeps its suffix`() {
    ...
    fun `test several targets are split on whitespace`() {
        assertEquals(
    ...
    fun `test a quoted target with a space stays one argument`() {
        assertEquals(
    ...
    fun `test a directory target is left alone`() {
        assertEquals(listOf("pytest", "-v", "tests"), ByPytest.arguments("tests"))
    }

    fun `test a target that already names a py file is left alone`() {
        assertEquals("tests/test_x.py::test_a", ByPytest.nodeId("tests/test_x.py::test_a"))
    }

    fun `test only the trailing extension is rewritten`() {
    ...
    fun `test the full command line puts the version flag before the module`() {
    ...
    fun `test extra args are forwarded to pytest after the targets`() {
